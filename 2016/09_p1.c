...
char **getfilelines(char *filename, int *numLines)
{
  ...
  int n = 100000; // max line length, increase this if it breaks
  char buffer[n];
...
int main(int argc, char **argv)
{
  ...
  for (int i = 0; i < lineLen; i++)
  {
    ...
    {
      char ts[100] = {'\0'};
      int readAhead = atoi(line + i + 1);
      int readAheadLen = strlen(itoa(readAhead, ts));
      if (line[i + readAheadLen + 1] == 'x')
      {
        int repeats = atoi(line + i + 1 + readAheadLen + 1);
        int repeatsLen = strlen(itoa(repeats, ts));
        if (line[i + readAheadLen + 1 + repeatsLen + 1] == ')')
        {
          char *repeatingSection = malloc(sizeof(char) * (readAhead + 1));
          strncpy(repeatingSection, line + i + 1 + readAheadLen + 1 + repeatsLen + 1, readAhead);
          repeatingSection[readAhead] = '\0';
          for (int times = 0; times < repeats; times++)
          {
            strcat(finalString, repeatingSection);
          }
          i += readAhead + readAheadLen + repeatsLen + 2;
        }
      }
    }
    else
    {
      finalString[strlen(finalString)] = c;
    }
  }
