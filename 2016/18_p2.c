...
bool map[400000][100] = {{false}};
char firstRow[] = ".^^^^^.^^.^^^.^...^..^^.^.^..^^^^^^^^^^..^...^^.^..^^^^..^^^^...^.^.^^^^^^^^....^..^^^^^^.^^^.^^^.^^";
int sum = 0;

bool isTrap(int rowIndex, int col)
{
  bool *row = map[rowIndex - 1];
  bool left;
  if (col == 0)
    left = false;
  else
  ...
  if (col == 99)
    ...
    right = row[col + 1];
  bool lcrn = left && center && !right;
  bool crln = center && right && !left;
  bool lcnrn = left && !center && !right;
  bool rcnln = right && !center && !left;
  return (lcrn || crln || lcnrn || rcnln);
}
