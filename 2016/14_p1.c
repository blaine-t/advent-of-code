...
char tripleCheck(char *string, int strLen)
{
  char prevPrevChar;
  char prevChar;
...
bool quintupleCheck(char cCheck, char *string, int strLen)
{
  char prevChars[4];
  for (int i = 0; i < strLen; i++)
  {
    char c = string[i];
    if (cCheck == c && cCheck == prevChars[3] && cCheck == prevChars[2] && cCheck == prevChars[1] && cCheck == prevChars[0])
    {
      return true;
    }
    for (int i = 0; i < 3; i++)
...
int main(int argc, char **argv)
{
  ...
  const int hashLen = 33;
  const int range = 1000;
  char indexStr[intLen];
  for (int index = 0; index < INT_MAX; index++)
  {
    itoa(index, indexStr, 10);
    char input[saltLen + intLen + 1];
    strlcpy(input, salt, sizeof(input));
    strlcat(input, indexStr, sizeof(input));
    char *hash = md5(input);
    char tc = tripleCheck(hash, hashLen);
    if (tc != '\0')
    {
      printf("%d\n", index);
      for (int i = 0; i < 1000; i++)
      {
        if (quintupleCheck(canidates[totIndex], hash, hashLen))
        {
          canidates[totIndex] = ' ';
          printf("%d\n", index);
          count++;
      ...
      canidates[totIndex] = tc;
      canIndex[totIndex] = index;
      totIndex++;
      totIndex %= 1000;
    }
  }
